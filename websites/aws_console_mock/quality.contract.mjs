 ...
 *
 * Not all routes are literal: detail pages and migrated list pages are generated by mapping over
 * the registry, so a gate that greps for path="..." reports them as missing. Both the generators
 * are checked for here rather than assumed — if either is deleted, the routes vanish wholesale
 * and every gate using this must fail.
 */
function routedPaths() {
  const app = read('src/App.jsx');
  const reg = read('src/lib/resourceRegistry.js');
  const literal = new Set([...app.matchAll(/path="([^"]+)"/g)].map((m) => m[1]));

  const detailGen = /RESOURCES\.map\([\s\S]{0,220}?path=\{r\.detailRoute\}/.test(app);
  const listGen = /RESOURCES\.filter\(\(r\) => r\.list\)[\s\S]{0,220}?path=\{r\.listRoute\}/.test(app);

  // Split the registry into per-resource blocks so a `list:` block is attributed correctly.
  const blocks = reg.split(/\n  \{\n    id: '/).slice(1);
  for (const b of blocks) {
    const listRoute = (b.match(/listRoute:\s*'([^']+)'/) || [])[1];
    const detailRoute = (b.match(/detailRoute:\s*'([^']+)'/) || [])[1];
    if (detailGen && detailRoute) literal.add(detailRoute);
    if (listGen && listRoute && /\n    list: \{/.test(b)) literal.add(listRoute);
  }
  return { paths: literal, detailGen, listGen };
}
...
await check('A1', 'every route the reference requires exists in the route table', () => {
  ...
  // Uses the shared resolver: a migrated list page is served by a generated <Route>, so greping
  // for a literal path="..." would report it as unimplemented the moment it improves.
  const { paths } = routedPaths();
  const missing = referenceRoutes(text).filter((r) => !paths.has(r));
...
await check('S8', 'every registry route is actually routed', () => {
  ...
  // renders its own h1, so even the rendered-title crawl read it as a healthy page. A link that
  // silently goes somewhere else is worse than one that errors.
  const { paths, detailGen } = routedPaths();
  assert(detailGen, 'App.jsx no longer generates <Route path={r.detailRoute}> from RESOURCES');
  const reg = read('src/lib/resourceRegistry.js');
  const wanted = [...new Set([
    ...[...reg.matchAll(/listRoute:\s*'([^']+)'/g)].map((m) => m[1]),
    ...[...reg.matchAll(/detailRoute:\s*'([^']+)'/g)].map((m) => m[1]),
  ])];
  const missing = wanted.filter((r) => !paths.has(r));
  assert(missing.length === 0, `not routed: ${missing.join(', ')}`);
  return `${wanted.length} registry routes, all served`;
});
...
await check('S9', 'the crawlers resolve generated routes, not just literal ones', () => {
  // This blind spot has now appeared three times: gate S8, gate A1, and depth-report.mjs each
  // grepped App.jsx for path="..." and so went blind to a page the moment it migrated to the
  // shared component. An instrument that silently stops covering improved code is worse than a
  // missing one, because its output still looks complete.
  const dr = read('depth-report.mjs');
  assert(/resourceRegistry/.test(dr), 'depth-report.mjs no longer reads the registry');
  assert(/r\.list\)\.map\(\(r\) => r\.listRoute\)/.test(dr),
    'depth-report.mjs no longer adds generated list routes to its crawl');
  const contract = read('quality.contract.mjs');
  assert(/function routedPaths\(\)/.test(contract), 'the shared route resolver is gone');
  for (const gateId of ["check('A1'", "check('S8'"]) {
    const body = contract.slice(contract.indexOf(gateId), contract.indexOf(gateId) + 1400);
    assert(/routedPaths\(\)/.test(body), `${gateId} no longer uses the shared route resolver`);
  }
  return 'depth-report and both route gates resolve generated routes';
});
